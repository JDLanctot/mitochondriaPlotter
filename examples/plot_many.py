...
def main(options: Options):
    ...
    if options.model == 'gen_lattice':
        ...
        edgelist = gen_lattice(options.lattice_size[0], options.lattice_size[1], options.p, options.k)
        ...
        if options.model == 'spatial':
            ...
            for p in p_values:
                for run in range(1, options.samples + 1):
                    save_path = get_image_path(options.model, dim, tau, p)
                    makedirs(save_path, exist_ok=True)

                    edgelist = load_edgelist(options.model, a1, a2, p, run, dim=options.dim, tau=options.tau)
                    fig = plot_network_from_edgelist(edgelist, options.model)
                    save_name = f"{options.file_name}_dim_{dim}_tau_{tau}_run{run}.png"
