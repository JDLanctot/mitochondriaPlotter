...
class Options:
    ...
    model: str = field(alias='-m', required=True, help="Model type: 'aspatial' or 'spatial'")
    data_type: str = field(alias='-t', required=True, help="Data type: 'cycles', 'components', or 'degrees'")
    ...
    input_file: str = field(alias='-i', required=False, default='analysis_results', help="Name of input CSV file")
...
def process_csv(options: Options):
    ...
    # Read the CSV file
    df = load_dataframe(options.model, a1, a2, dim=options.dim, tau=options.tau)
    ...
    if options.data_type == 'cycles':
        ...
        raise ValueError("Invalid data type. Choose 'cycles', 'components', or 'degrees'.")
    ...
    output_dir = save_dir(options.model, a1, a2, dim=options.dim, tau=options.tau)
    ...
    with open(output_path, 'w') as f:
        for index, row in df.iterrows():
            x_value = x_col.iloc[index] if isinstance(x_col, pd.Series) else x_value
            para_value = row[para_col]
            data_values = [row[col] for col in data_cols]
            line = f"{para_value} {x_value} " + " ".join(map(str, data_values)) + "\n"
            f.write(line)
